...
class TestNotebookLMBridge:
    ...
    @patch("metascaffold.notebooklm_bridge._get_client")
    async def test_query_graceful_degradation_on_error(self, mock_get_client):
        ...
        result = await bridge.query("test")
        assert result.success is False
        assert "Auth expired" in result.reason

    @patch("metascaffold.notebooklm_bridge._get_client")
    async def test_query_raises_when_fallback_disabled(self, mock_get_client):
